-- Migration 017: Use issue created_at for duplicate 24h window
--
-- Changes duplicate_issues 24h window from:
--   WHERE di.recorded_at >= NOW() - INTERVAL '24 hours'
-- To:
--   JOIN github_issues gi ... WHERE gi.created_at >= NOW() - INTERVAL '24 hours'
--
-- This ensures duplicates are counted based on when the issue was created on GitHub,
-- not when the system detected/recorded it as a duplicate.
...
-- RECORD MIGRATION
-- ============================================================================
INSERT INTO schema_migrations (version, name) VALUES (17, 'duplicate_use_created_at')
